...
LedControl altitude=LedControl(13,12,11,2);
...
const int RCSDirF = 36; //set to 36
...
int lastDebounceTimeRCSTRANSLATION = lastDebounceTime;
...
void setup() {
  ...
  // Set up Seven Segment Displays

  for(int index=0;index<altitude.getDeviceCount();index++) {
        altitude.shutdown(index,false); 
  }
...
void loop() {
  ...
  if ((millis() - lastDebounceTimeRCSTRANSLATION) > debounceDelay) {
    // // RCS Right
    ...
    //   mySimpit.send(KEYBOARD_EMULATOR, msg);
    //   jPressed = true;
    //   lastDebounceTimeRCSTRANSLATION=millis();
    // }else if (jPressed && !readingRCSL) {
